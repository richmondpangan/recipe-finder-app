...
function MainCourse() {
  ...
  const getMainCourse = async () => {
    ...
    if (check) {
      ...
      const api = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${import.meta.env.VITE_REACT_APP_API_KEY}&query=main+course&number=20`);
      ...
      localStorage.setItem('main-course', JSON.stringify(randomRecipes));
      setMainCourse(randomRecipes);
