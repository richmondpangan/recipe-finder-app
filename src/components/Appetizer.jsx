...
function Appetizer() {
    ...
    const getAppetizer = async () => {
      ...
      if (check) {
        ...
        const api = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${import.meta.env.VITE_REACT_APP_API_KEY}&query=appetizer&number=10`);
        const data = await api.json();
  
        localStorage.setItem('appetizer', JSON.stringify(data.results));
